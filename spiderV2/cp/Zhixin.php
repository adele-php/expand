...
class Zhixin extends SpiderAbstract {
    const CPID = '39';
    const SECRETKEY = 'zhangdu520fwefew534266!';
    const BASE_URL = 'http://www.zhixinbook.cn/api/';

    public function init() {
        $this->source = Book::SOURCE_ZHIXIN;
...
//        $data = $this->getChapterList(11807);
        $data = $this->getChapterInfos([11807], [1194440]);
    ...
    public function getBookList() {
        ...
        $url = self::BASE_URL . 'book.php' . $this->buildQuery($param);
        $data = $this->curl->run([
            CURLOPT_URL => $url
        ]);
        ...
        if($data){
            $list = [];
            foreach ($data as $k => $v){
                if ($book_detail = $this->getBookDetail($v['id'])) {
                    $book_detail['book_update_time'] = $v['lastupdate'];
                    $list[$book_detail['id']] = $book_detail;
                }
            }
            return $list;
        }
    ...
    public function getBookDetail($interface_bid) {
        $param = [
            'cpid' => self::CPID,
            'book_id' => $interface_bid,
        ];
        $url = self::BASE_URL . 'bookInfo.php';
    ...
    public function getChapterList($interface_bid) {
        ...
        $url = self::BASE_URL . 'chapterList.php';
        $url .= $this->buildQuery($param);
        $data = $this->curl->run([
            CURLOPT_URL => $url
        ]);
    ...
    public function getChapterInfos($interface_bids, $interface_cids) {
        foreach ($interface_bids as $k => $interface_bid) {
            $param = [
                'cpid' => self::CPID,
                'book_id' => $interface_bid,
                'chapter_id' => $interface_cids[$k],
            ];
            $url = self::BASE_URL . 'content.php';
            $url .= $this->buildQuery($param);
            $opt[] = [
                CURLOPT_URL => $url,
                'id' => $k
        ...
        if ($data) {
            ...
            foreach ($data as $k => $v) {
                ...
                $v = [
                    ...
                    'pic' => $v['cover'],
                    'intro' => $v['brief'],
                    'is_serial' => $v['complete_status'] == 1 ? 1 : 0,
                    'author' => $v['author'],
                    'source' => $this->source
                ];
    ...
    public function gatherChapter($book_ids) {
        ...
        while (true) {
            ...
            foreach ($chapter_lists as $chapter_list) {
                $sort ++;
                if (!TmpChapter::getModelByCondition('interface_chapter_id=:ici and source=:source and interface_book_id=:book_id', [':ici' => $chapter_list['id'], ':source' => $this->source, ':book_id' => $tmp_book['book_id']], 'id')) {
                    $chapter_infos[] = [
                        'sort' => $sort,
                        'vid' => 0,
                        'vname' => '正文',
                        'cid' => $chapter_list['id'],
                        'cname' => $chapter_list['name'],
                    ];
                }
            }
            //章节列表入库
