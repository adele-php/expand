...
class Changdu extends SpiderAbstract {
    const CORPID = 'hzzd';
    const BASE_URL = 'http://api.yuedunovel.com/chiwen/index.asp';
    const DEBUG = false;

    public function init() {
        $this->source = Book::SOURCE_CHANGDU;
...
//        $data = $this->getChapterInfos([10050], [1840]);
    ...
    public function getBookList($time = 0) {
        ...
        $url = self::BASE_URL . $this->buildQuery($param);
        $data = $this->curl->run([
            CURLOPT_URL => $url
        ]);
        $data = $this->checkResult($data);

        if(isset($data['bookitem'])){
            $list = [];
            foreach ($data['bookitem'] as $k => $v){
    ...
    public function gatherChapter($book_ids) {
        ...
        while (true) {
            ...
            foreach ($chapter_lists as $chapter_list) {
                if (!TmpChapter::getModelByCondition('interface_chapter_id=:ici and source=:source and interface_book_id=:book_id', [':ici' => $chapter_list['chapterId'], ':source' => $this->source, ':book_id' => $chapter_list['bookId']], 'id')) {
                    $chapter_infos[] = [
    ...
    private function checkResult($data) {
        $object = simplexml_load_string($data, 'SimpleXMLElement', LIBXML_NOCDATA | LIBXML_NOBLANKS);
